...
$bdd= new Database();
$rechercheBebe=$_POST['rechercheBebe'];
$ligne=$bdd->requete("SELECT id_bebe,id_operation, GROUP_CONCAT(valeur ORDER BY element) AS groupement FROM protocoleA WHERE element IN('heure de délivrance','heure d\'extraction','nom du bébé','Prenom du bébé','Sexe','Etat') GROUP BY id_bebe HAVING id_bebe LIKE '%$rechercheBebe%'");
	...
	{	
		$nomPatiente = $fetch1['nom_patiente'];
  		echo " <tr>
					...
					<td class='col-lg-2 col-md-2 col-sm-2 col-xs-2'style='padding-left:15px'>$naissance</td>
