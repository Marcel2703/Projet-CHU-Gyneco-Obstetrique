...
$bdd= new Database();
$patiente=$bdd->requete("SELECT patiente.id_registre AS id_registre,patiente.nom_patiente AS nom_patiente,patiente.gestite AS gestite,patiente.parite AS parite, DATE_FORMAT(renseignement.naissance, '%d/%m/%Y') AS naissance,renseignement.lieu AS lieu,renseignement.pere AS pere,renseignement.mere AS mere, renseignement.domicile AS domicile, renseignement.croyance AS croyance, renseignement.profession AS profession, renseignement.situation AS situation FROM patiente,renseignement WHERE patiente.id_registre=renseignement.id_registre");
    ...
    $situation = $fetch['situation'];
      ...
      else
      {
            ...
            while( $fetch = $sortie->fetch(PDO::FETCH_ASSOC) ){
              ...
              if($dateSortie=='00/00/0000'){
                $dateSortie='Pas encore sortie';
              }
            }
              ...
              while( $fetch = $operation->fetch(PDO::FETCH_ASSOC) )
              {
                ...
                $chaineBebe="";
                  while( $fetch = $protocole->fetch(PDO::FETCH_ASSOC) )
                  {
                    ...
                    $bebe=$bdd->requete("SELECT GROUP_CONCAT(element) AS groupementElement,GROUP_CONCAT(valeur) AS groupementValeur FROM protocoleA WHERE id_bebe='{$idBebe}' AND element IN('date de délivrance','date d\'extraction','heure de délivrance','Heure d\'extraction','nom du bébé','Prenom du bébé','Sexe','Etat','Poids','pt','pc','pb','t')");
                    ...
                    $protocoleDetails=$bdd->requete("SELECT GROUP_CONCAT(element) AS groupementElementPro,GROUP_CONCAT(valeur) AS groupementValeurPro FROM protocoleA WHERE id_bebe='{$idBebe}' AND element NOT IN('date de délivrance','date d\'extraction','heure de délivrance','Heure d\'extraction','nom du bébé','Prenom du bébé','Sexe','Etat','Poids','pt','pc','pb','t')");
                      ...
                      $j=0;
                      while ( $j<$taille1 AND $tabElement[$j]!='') {
