...
$bdd= new Database();
$rechercheFiche=$_POST['rechercheFiche'];
$ligne=$bdd->requete("SELECT operation.id_registre AS id_registre,patiente.nom_patiente AS nom_patiente FROM operation,patiente WHERE (operation.id_registre=patiente.id_registre AND operation.id_registre LIKE '%$rechercheFiche%') OR patiente.nom_patiente LIKE '%$rechercheFiche%'");
