...
if(isset($_SESSION['idOperation'])){
	...
	$ligne=$bdd->requete("SELECT protocoleA.id_operation AS id_operation, protocoleA.id_bebe AS id_bebe, protocoleA.element AS element, protocoleA.valeur AS valeur FROM protocoleA,operation WHERE (protocoleA.id_operation='{$idOperation}' AND operation.id_operation='{$idOperation}') AND operation.id_acte='2'");
	...
	$idBebe = $fetch['id_bebe'];
  	...
  	$valeur = $fetch['valeur'];
  	if ($element=='Date d\'extraction' || $element=='Heure d\'extraction' || $element=='Nom du bébé' || $element=='Prenom du bébé' || $element=='Sexe' || $element=='Etat')
  	{
  			echo " <tr>
