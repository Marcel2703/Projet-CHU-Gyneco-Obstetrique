...
$bdd= new Database();
$rechercheSejour=$_POST['rechercheSejour'];
$ligne=$bdd->requete("SELECT *,DATE_FORMAT(date_entree, '%d/%m/%Y') AS dateFormEntree,DATE_FORMAT(date_sortie, '%d/%m/%Y') AS dateFormSortie FROM sejour WHERE id_sejour LIKE '%$rechercheSejour%' OR id_chambre LIKE '%$rechercheSejour%' OR id_lit LIKE '%$rechercheSejour%' OR id_registre LIKE '%$rechercheSejour%' OR nom_patiente LIKE '%$rechercheSejour%' ORDER BY id_sejour DESC");
