...
$bdd= new Database();
$where="";
$recherchePerso=$_POST['recherchePerso'];
$recherchePersoTab= preg_split('/[\s\-]/',$recherchePerso);
$taille=count($recherchePersoTab);
	foreach ($recherchePersoTab as $key => $value) 
	{
		$where .="nom_perso LIKE '%$value%'";
		if($key !=($taille-1))
		{
			$where .=" AND ";
		}
		...
		{
			$where .="OR poste LIKE '%$recherchePerso%' OR id_perso LIKE '%$recherchePerso%'";
		}

	}
$ligne=$bdd->requete("SELECT * FROM personnel WHERE $where ORDER BY nom_perso");
